        ...
        if (results.length === 0) {
            ...
            searchResults.innerHTML = results.map(result => `
                <div class="search-result">
                    <h4><a href="${result.url}">${highlightText(result.originalTitle, query)}</a></h4>
                    ${result.module ? `<span class="search-module">Module: ${result.module}</span>` : ''}
                    <p>${highlightText(truncateText(result.originalContent, query), query)}</p>
                </div>
            `).join('');
        }
